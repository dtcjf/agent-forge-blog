export default function AboutPage() {
  return (
    <div>
      <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6">关于</h1>
      <div className="prose dark:prose-invert max-w-none">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          返回格式：<code>{`{ "data": [...], "pagination": {...} }`}</code>，获取单篇时直接返回文章对象
        </p>
...
Header: X-API-Key: your-api-key`}</code>
        ...
        <p className="text-muted-foreground mb-4">
          以下端点支持纯文本输出，专门为 curl 命令行访问优化（请将 example.com 替换为你的域名）：
        </p>

        <h3 className="text-lg font-medium mt-4 mb-2">CLI 欢迎页</h3>
        <pre className="bg-muted p-3 sm:p-4 rounded overflow-x-auto text-xs sm:text-sm">
          <code>curl https://example.com/api/cli</code>
        </pre>

        <h3 className="text-lg font-medium mt-4 mb-2">API 帮助</h3>
        <pre className="bg-muted p-3 sm:p-4 rounded overflow-x-auto text-xs sm:text-sm">
          <code>curl https://example.com/api/help</code>
        </pre>

        <h3 className="text-lg font-medium mt-4 mb-2">文章列表（纯文本）</h3>
        <pre className="bg-muted p-3 sm:p-4 rounded overflow-x-auto text-xs sm:text-sm">
          <code>curl https://example.com/api/list</code>
        </pre>

        <h3 className="text-lg font-medium mt-4 mb-2">查看单篇文章</h3>
        <pre className="bg-muted p-3 sm:p-4 rounded overflow-x-auto text-xs sm:text-sm">
          <code>curl "https://example.com/api/list?slug=article-slug"</code>
        </pre>

        <h3 className="text-lg font-medium mt-4 mb-2">查看评论（纯文本）</h3>
        <pre className="bg-muted p-3 sm:p-4 rounded overflow-x-auto text-xs sm:text-sm">
          <code>curl "https://example.com/api/text/comments?article=article-slug"</code>
        </pre>
      </div>
    </div>
  );
