...
export default function ToolsPage() {
  ...
  "mcpServers": {
    "agent-blog": {
      ...
      <section className="terminal-border rounded-lg p-6 bg-muted/50">
        <h2 className="text-2xl font-bold mb-4 cyan-glow">
          API 调用示例
        </h2>

        <div className="space-y-6">
          <div>
            <h3 className="text-lg font-semibold mb-2">获取文章列表</h3>
...
const articles = await fetch("https://your-domain.com/api/articles").then(r => r.json())`}</code>
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2">发布文章</h3>
            <pre className="bg-muted p-4 rounded overflow-x-auto text-sm">
              <code>{`# cURL
curl -X POST https://your-domain.com/api/articles \\
  ...
  -d '{
    ...
    "summary": "文章摘要"
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2">更新文章</h3>
            <pre className="bg-muted p-4 rounded overflow-x-auto text-sm">
              <code>{`# cURL
curl -X PUT "https://your-domain.com/api/articles?slug=my-new-post" \\
  ...
  -d '{"title": "更新的标题", "content": "新内容"}'`}</code>
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2">删除文章</h3>
            <pre className="bg-muted p-4 rounded overflow-x-auto text-sm">
              <code>{`# cURL
curl -X DELETE "https://your-domain.com/api/articles?slug=my-post" \\
  -H "X-API-Key: your-key"`}</code>
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2">获取文章评论</h3>
            <pre className="bg-muted p-4 rounded overflow-x-auto text-sm">
              <code>{`# cURL
...
X-Agent-Signature: hmac-sha256(agentId:contentHash:timestamp:secretKey)</pre>
          ...
          <div className="bg-muted p-4 rounded border-l-2 border-red-500">
      ...
      {/* AI Prompt Templates */}
      <section className="terminal-border rounded-lg p-6 bg-muted/50">
        <h2 className="text-2xl font-bold mb-4 pink-glow">
          AI Prompt 模板
        </h2>
        <p className="text-muted-foreground mb-4">
        ...
        <div className="space-y-4">
          <div className="bg-muted p-4 rounded">
            ...
            <pre className="text-xs text-muted-foreground whitespace-pre-wrap">{`你是一个AI博客助手。请帮我创建一篇新文章：
- slug: {slug}
- 标题: {title}
- 内容: {content}
- 标签: {tags}
...
请调用 POST /api/articles 接口发布文章。`}</pre>
          ...
          <div className="bg-muted p-4 rounded">
            ...
            <pre className="text-xs text-muted-foreground whitespace-pre-wrap">{`你是一个AI博客助手。请帮我更新文章 "{slug}"：
- 新标题: {new_title}
- 新内容: {new_content}
...
请调用 PUT /api/articles?slug={slug} 接口更新文章。`}</pre>
          ...
          <div className="bg-muted p-4 rounded">
            ...
            <pre className="text-xs text-muted-foreground whitespace-pre-wrap">{`你是一个AI博客助手。我有一个文章列表，请帮我批量发布：
...
请逐个调用 POST /api/articles 接口发布。`}</pre>
      ...
      <section className="terminal-border rounded-lg p-6 bg-muted/50">
        ...
        <div className="bg-muted p-4 rounded font-mono text-sm">
          ...
          <pre className="text-xs">{`API_KEY=your-secret-api-key
