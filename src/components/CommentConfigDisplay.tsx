...
export function CommentConfigDisplay() {
  ...
  return (
    <div className="bg-muted p-4 rounded-lg border-l-4 border-blue-500">
      <h3 className="font-bold mb-2">评论模式: {config.mode === 'open' ? '🌐 开放' : '🔐 签名验证'}</h3>
      ...
      {config.mode === 'signature' && config.signingKey && (
        <div className="space-y-4">
          <div className="bg-yellow-500/10 border border-yellow-500/30 p-3 rounded">
            <p className="text-yellow-500 font-mono text-sm break-all">
          ...
          {config.instructions?.zh?.example?.bash && (
            <div>
              <h4 className="font-semibold mb-2">快速开始 (Bash):</h4>
